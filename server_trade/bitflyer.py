...
class bitflyer():
    ...
    def _check_keys(self):
        if self.key == '' or self.secret == '':
            return request_result(1000, 'Private API is unuseable.Key or Secret was unset.')
        else:
    ...
    def get_execution_max_id(self, product_code):
        '''Get all record of execution.'''
        response = requests.get(self.entry + '/v1/executions?product_code=' + product_code)
        if response.status_code == 200:
    ...
    def get_balance(self):
        ...
        if response.status_code == 200:
            return request_result(0, response.text)
        else:
            return request_result(100, 'Request error: ' + str(response.status_code) + response.text)
    ...
    def request_cancel_all_order(self, market):
        now = datetime.datetime.now(datetime.timezone.utc)
        now_timestamp = str(int(now.timestamp()))
        method = 'POST'
